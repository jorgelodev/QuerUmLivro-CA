...
{
    public class InteresseLivroEstaDisponivelSpecTests
    {
        private readonly InteresseLivroEstaDisponivelSpec _specification;
        private readonly Faker<Interesse> _interesseFaker;

        public InteresseLivroEstaDisponivelSpecTests()
        {
            var livroGatewayMock = new Mock<ILivroGateway>();
            _specification = new InteresseLivroEstaDisponivelSpec(livroGatewayMock.Object);
        ...
        [Fact]
        public void ReturnsTrue_WhenLivroIsAvailable()
        {
            // Arrange
            var interesse = _interesseFaker.Generate();
            var livro = new Livro("Livro Teste", 1) { Disponivel = true };
            var livroGatewayMock = new Mock<ILivroGateway>();
            livroGatewayMock.Setup(gateway => gateway.ObterPorId(interesse.LivroId)).Returns(livro);
            var spec = new InteresseLivroEstaDisponivelSpec(livroGatewayMock.Object);
        ...
        [Fact]
        public void ReturnsFalse_WhenLivroIsNotAvailable()
        {
            // Arrange
            var interesse = _interesseFaker.Generate();
            var livro = new Livro("Livro Teste", 1) { Disponivel = false };
            var livroGatewayMock = new Mock<ILivroGateway>();
            livroGatewayMock.Setup(gateway => gateway.ObterPorId(interesse.LivroId)).Returns(livro);
            var spec = new InteresseLivroEstaDisponivelSpec(livroGatewayMock.Object);
        ...
        public void ErrorMessage_ShouldReturnCorrectMessage()
        {
            ...
            // Assert
            errorMessage.Should().Be("Livro não está mais disponível.");
        }
    }
}
